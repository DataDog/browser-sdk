...
export default class AgentClient {
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        const error = new Error(
          `Cannot connect to agent at ${this.agentUrl} - connection refused. Is the agent running?`
        )
  ...
  _parseConfigUpdate(data) {
    ...
    if (toApply.length > 0 || toModify.length > 0 || toUnapply.length > 0) {
      console.log(
        `[AgentClient] Config changes: +${toApply.length} new, ~${toModify.length} modified, -${toUnapply.length} removed (total: ${this.appliedConfigs.size})`
      )
    }
