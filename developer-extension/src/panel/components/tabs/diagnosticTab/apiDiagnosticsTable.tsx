...
export function ApiDiagnosticsTable() {
  ...
  switch (apiDiagnostics?.status) {
    ...
    case 'success':
      return <Flex direction='column' style={{ rowGap: '1em', paddingTop: '1em' }}>
...
function ApiDiagnosticRow({ diagnostic }: { diagnostic: ApiDiagnostic }) {
  return <Flex direction='row' style={{ columnGap: '5px' }}>
    <ApiDiagnosticBadge level={diagnostic.level} />
    <Card>
      <ApiDiagnosticDescription
...
function ApiDiagnosticDescription({ subject, message }: {
  ...
  const components = (
    subject.length >= 2 &&
    subject[0].name === 'window' &&
    subject[1].type !== 'value'
  ...
  const useCompactPath = (components.length === 1) ||
    (components.length === 2 && components[0].type === 'value')
...
function ApiDiagnosticCompactPath({ components }: { components: ApiPathComponent[] }) {
  return <Text style={{ fontWeight: 'bold' }}>
    {components.map(formatPathComponentText).join('.')}
  </Text>
}

function ApiDiagnosticExpandedPath({ components }: { components: ApiPathComponent[] }) {
  return components.map((component, index) => (
    <ApiDiagnosticPathComponent component={component} index={index} />
  ))
}

function ApiDiagnosticPathComponent({ component, index }: {
  component: ApiPathComponent,
  index: number
}) {
  const leadingDot = index === 0 ? '' : '.'
  const text = `${leadingDot}${formatPathComponentText(component)}`
  const marginLeft = index === 0 ? '0' : '1em'
  return <Text style={{ fontWeight: 'bold', marginLeft }}>
    {text}
  </Text>
}
