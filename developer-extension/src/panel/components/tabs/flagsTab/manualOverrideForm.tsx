...
import { FLAG_TYPES, FLAG_TYPE_CONFIG, parseTypedString, validateOverrideValue, type FlagType } from './flagTypes'
...
export function ManualOverrideForm() {
  ...
  function submit() {
    ...
    if (catalogFlag && catalogFlag.type !== type) {
      setError(
        `"${trimmedKey}" is a ${FLAG_TYPE_CONFIG[catalogFlag.type].label} flag in the catalog — use that type instead`
      )
      return
    }
