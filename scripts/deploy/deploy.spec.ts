...
describe('deploy', () => {
  ...
  it('should deploy root packages', async () => {
    ...
    assert.deepEqual(getS3Commands(), [
      ...
      {
        // Profiler chunk
        command: `aws s3 cp --cache-control max-age=14400, s-maxage=60 packages/rum/bundle/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-prod/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        ...
        command: `aws s3 cp --cache-control max-age=14400, s-maxage=60 packages/rum/bundle/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-prod/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      // RUM bundle
      {
    ...
    assert.deepEqual(getCloudfrontCommands(), [
      {
        command: `aws cloudfront create-invalidation --distribution-id EGB08BYCT1DD9 --paths /datadog-logs-v6.js,/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js,/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js,/datadog-rum-v6.js,/datadog-rum-slim-v6.js`,
        env,
      },
    ])
  })
  ...
  it('should deploy datacenter packages', async () => {
    ...
    assert.deepEqual(getS3Commands(), [
      ...
      // RUM Profiler Chunk
      {
        command: `aws s3 cp --cache-control max-age=14400, s-maxage=60 packages/rum/bundle/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-prod/us1/v6/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command: `aws s3 cp --cache-control max-age=14400, s-maxage=60 packages/rum/bundle/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-prod/us1/v6/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command:
    ...
    assert.deepEqual(getCloudfrontCommands(), [
      {
        command: `aws cloudfront create-invalidation --distribution-id EGB08BYCT1DD9 --paths /us1/v6/datadog-logs.js,/us1/v6/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js,/us1/v6/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js,/us1/v6/datadog-rum.js,/us1/v6/datadog-rum-slim.js`,
        env,
      },
    ])
  })
  ...
  it('should deploy staging packages', async () => {
    ...
    assert.deepEqual(getS3Commands(), [
      ...
      // RUM Profiler Chunk
      {
        command: `aws s3 cp --cache-control max-age=900, s-maxage=60 packages/rum/bundle/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-staging/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command: `aws s3 cp --cache-control max-age=900, s-maxage=60 packages/rum/bundle/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-staging/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command:
    ...
    assert.deepEqual(getCloudfrontCommands(), [
      {
        command: `aws cloudfront create-invalidation --distribution-id E2FP11ZSCFD3EU --paths /datadog-logs-staging.js,/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js,/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js,/datadog-rum-staging.js,/datadog-rum-slim-staging.js`,
        env,
      },
    ])
  })
  ...
  it('should deploy PR packages', async () => {
    ...
    assert.deepEqual(getS3Commands(), [
      ...
      // RUM Profiler Chunk
      {
        command: `aws s3 cp --cache-control max-age=900, s-maxage=60 packages/rum/bundle/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-staging/pull-request/123/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command: `aws s3 cp --cache-control max-age=900, s-maxage=60 packages/rum/bundle/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js s3://browser-agent-artifacts-staging/pull-request/123/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js`,
        env,
      },
      {
        command:
    ...
    assert.deepEqual(getCloudfrontCommands(), [
      {
        command: `aws cloudfront create-invalidation --distribution-id E2FP11ZSCFD3EU --paths /pull-request/123/datadog-logs.js,/pull-request/123/chunks/datadog-profiler_${FAKE_CHUNK_HASH}-datadog-rum.js,/pull-request/123/chunks/datadog-recorder_${FAKE_CHUNK_HASH}-datadog-rum.js,/pull-request/123/datadog-rum.js,/pull-request/123/datadog-rum-slim.js`,
        env,
      },
    ])
  })
