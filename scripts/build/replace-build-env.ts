...
runMain(async () => {
  ...
  for (const filePath of globSync('**/*.js', { cwd: buildDirectory, absolute: true })) {
    if (await modifyFile(filePath, (content: string) => replaceBuildEnv(content))) {
      printLog(`Replaced BuildEnv in ${filePath}`)
    }
  }
})
