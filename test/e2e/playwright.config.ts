...
const baseWebServers = [
  ...
  },
  {
    ...
    // This app is generated by build:apps and gitignored, so it is not registered in the
    // Yarn workspace. Unlike the sibling nuxt-app entry, `yarn dev` fails here because Yarn 4
    // traverses up to the workspace root and can't find the package. Use `yarn start` (nuxt
    // preview of the pre-built .output/) unconditionally — it runs via node, not yarn scripts.
    command: 'yarn start',
