...
//
// Usage: node pinned-proxy.mjs --listen 5400 --upstream 127.0.0.1:5401
...
httpServer.on('upgrade', (req, socket, head) => {
  ...
  wss.handleUpgrade(req, socket, head, (clientWs) => {
    ...
    const guidTypes = new Map()
    let queueToUpstream = []

    upstreamWs.on('open', () => {
      for (const msg of queueToUpstream) upstreamWs.send(msg)
      queueToUpstream = []
    })
    ...
    clientWs.on('message', (data) => {
      ...
      if (TRACE) console.log('C->S', text.slice(0, 500))
      ...
      else queueToUpstream.push(rewritten)
    })

    upstreamWs.on('message', (data) => {
      const text = data.toString()
      if (TRACE) console.log('S->C', text.slice(0, 800))
    ...
    upstreamWs.on('error', (err) => {
      console.error('[pinned-proxy] upstream error:', err.message)
      closeBoth()
    })
  })
})
...
httpServer.listen(LISTEN, '127.0.0.1', () => {
  console.log(`[pinned-proxy] listening on ws://127.0.0.1:${LISTEN}/ -> ws://${UPSTREAM}/`)
})

function forwardHeader(headers, name) {
  return headers[name] ? { [name]: headers[name] } : {}
}

// Server (1.40) -> Client (1.58)
function rewriteServerToClient(text, guidTypes) {
  let msg
  try { msg = JSON.parse(text) } catch { return text }
  if (msg && msg.method === '__create__' && msg.params) {
    const type = msg.params.type
    if (msg.params.guid) guidTypes.set(msg.params.guid, type)
    const init = msg.params.initializer || {}
    // Selectors channel was removed; drop the __create__ and strip references from Playwright.
    if (type === 'Selectors') return null
    if (type === 'Playwright') delete init.selectors
    // BrowserContextInitializer requires `options` in 1.58 (sub-fields all optional).
    if (type === 'BrowserContext' && init.options === undefined) init.options = {}
    // RequestInitializer requires `hasResponse` in 1.58.
    if (type === 'Request' && init.hasResponse === undefined) init.hasResponse = false
...
function rewriteClientToServer(text, guidTypes) {
  let msg
  try { msg = JSON.parse(text) } catch { return text }
  if (!msg || typeof msg.method !== 'string') return text
  const type = guidTypes.get(msg.guid)
  ...
  const noWaitAfterMethods = {
    Frame: new Set(['check', 'dblclick', 'dragAndDrop', 'fill', 'hover', 'selectOption', 'setInputFiles', 'tap', 'type', 'uncheck']),
    ElementHandle: new Set(['check', 'dblclick', 'fill', 'hover', 'selectOption', 'setInputFiles', 'tap', 'type', 'uncheck']),
  }
  if (noWaitAfterMethods[type]?.has(msg.method) && params.noWaitAfter === undefined) {
