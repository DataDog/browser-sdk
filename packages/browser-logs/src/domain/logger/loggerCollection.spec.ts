...
describe('logger collection', () => {
  ...
  describe('when handle type is set to "console"', () => {
    beforeEach(() => {
      ...
      vi.spyOn(originalConsoleMethods, 'debug').mockImplementation(() => undefined)
      vi.spyOn(originalConsoleMethods, 'info').mockImplementation(() => undefined)
      vi.spyOn(originalConsoleMethods, 'warn').mockImplementation(() => undefined)
      vi.spyOn(originalConsoleMethods, 'error').mockImplementation(() => undefined)
      vi.spyOn(originalConsoleMethods, 'log').mockImplementation(() => undefined)
    })
    ...
    it('should print the log message and context to the console', () => {
      ...
      expect(originalConsoleMethods.error).toHaveBeenCalledExactlyOnceWith('message', {
        foo: 'from-logger',
        bar: 'from-message',
      })
    })
