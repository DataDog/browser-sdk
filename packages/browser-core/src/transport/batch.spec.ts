...
describe('batch', () => {
  ...
  describe('adding a message', () => {
    it('should add message to the flush controller', () => {
      ...
      expect(flushController.notifyBeforeAddMessage).toHaveBeenCalledExactlyOnceWith(SMALL_MESSAGE_BYTES_COUNT)
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledTimes(1)
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledExactlyOnceWith(0)
    })
    ...
    it('should adjust the bytes count when replacing a message with the same key', () => {
      ...
      flushController.notifyAfterAddMessage.mockClear()

      batch.upsert(SMALL_MESSAGE, 'a')

      expect(flushController.notifyBeforeAddMessage).not.toHaveBeenCalled()
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledTimes(1)
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledExactlyOnceWith(0) // same size, diff = 0
      expect(flushController.bytesCount).toEqual(
    ...
    it('should adjust the message size after the message has been added', () => {
      ...
      batch.add(message)
      expect(flushController.notifyBeforeAddMessage).toHaveBeenCalledTimes(1)
      expect(flushController.notifyBeforeAddMessage).toHaveBeenCalledExactlyOnceWith(16)
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledTimes(1)
      expect(flushController.notifyAfterAddMessage).toHaveBeenCalledExactlyOnceWith(2) // 2 more bytes once encoded
    })
  })
  ...
  describe('flush messages when the page is not exiting or with a synchronous encoder', () => {
    ...
    it('should encode upserted messages', () => {
      const encoderWriteSpy = vi.spyOn(encoder, 'write').mockImplementation(() => undefined)
      ...
      expect(encoderWriteSpy).toHaveBeenCalledExactlyOnceWith('{"message":"2"}')
    })
