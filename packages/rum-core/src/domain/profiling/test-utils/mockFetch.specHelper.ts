export function mockFetch() {
  ...
  const fetchMock = jasmine
    .createSpy('fetchMock')
    .and.callFake((_input: RequestInfo | URL, _init?: RequestInit) => Promise.resolve(new Response()))
  ...
  function extractIntakeUrlAndFormDataFromFetch(callIndex = 0) {
    ...
    const [intakeUrl, request] = calls[callIndex] as unknown as [string, { body: FormData; method: string }]
