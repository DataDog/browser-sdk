import { DEFAULT_CONFIGURATION, isIE, objectEntries } from '@datadog/browser-core'
...
describe('tracer', () => {
  ...
  describe('traceXhr', () => {
    ...
    it('should add traceId and spanId to context and add tracing headers', () => {
      const tracer = startTracer(configuration as RumConfiguration, sessionManager)
    ...
    it('should not trace request during untracked session', () => {
      const tracer = startTracer(configuration as RumConfiguration, sessionManager.setNotTracked())
      const context = { ...ALLOWED_DOMAIN_CONTEXT }
      tracer.traceXhr(context, (xhrStub as unknown) as XMLHttpRequest)
    ...
    it('should trace requests on configured origins', () => {
      const configurationWithTracingUrls: Partial<RumConfiguration> = {
        ...configuration,
        allowedTracingOrigins: [/^https?:\/\/qux\.com/, 'http://bar.com'],
      }
      const stub = (xhrStub as unknown) as XMLHttpRequest

      const tracer = startTracer(configurationWithTracingUrls as RumConfiguration, sessionManager)
