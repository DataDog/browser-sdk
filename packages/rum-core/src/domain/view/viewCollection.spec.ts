...
describe('viewCollection', () => {
  ...
  describe('soft navigation correlation', () => {
    ...
    it('should annotate route_change view with is_soft_navigation when soft-nav entry exists at query time', () => {
      ...
      const rawRumViewEvent = rawRumEvents[rawRumEvents.length - 1].rawRumEvent as RawRumViewEvent
      expect(rawRumViewEvent.view.is_soft_navigation).toBe(true)
    })

    it('should annotate route_change view on subsequent update when soft-nav entry arrives late', () => {
      ...
      expect(firstEvent.view.is_soft_navigation).toBeUndefined()
      ...
      expect(secondEvent.view.is_soft_navigation).toBe(true)
    })
    ...
    it('should correctly attribute soft-nav entries to respective views during rapid navigations', () => {
      ...
      expect(viewAEvent.view.is_soft_navigation).toBe(true)
      ...
      expect(viewBEvent.view.is_soft_navigation).toBe(true)
    })

    it('should not annotate route_change view when no soft-nav entry exists (programmatic pushState)', () => {
      const softNavContexts: SoftNavigationContexts = {
        findSoftNavigationByTime: () => undefined,
        findAll: () => [],
      }
      ...
      const rawRumViewEvent = rawRumEvents[rawRumEvents.length - 1].rawRumEvent as RawRumViewEvent
      expect(rawRumViewEvent.view.is_soft_navigation).toBeUndefined()
    })
    ...
    it('should not annotate route_change view when feature is disabled (noop contexts)', () => {
