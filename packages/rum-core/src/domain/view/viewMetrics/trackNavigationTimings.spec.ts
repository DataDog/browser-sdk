...
import { mockClock, replaceMockableWithSpy, registerCleanupTask } from '@datadog/browser-core/test'
import type { RumPerformanceNavigationTiming } from 'packages/rum-core/src/browser/performanceObservable'
...
} satisfies RelevantNavigationTiming as RumPerformanceNavigationTiming

const FAKE_INCOMPLETE_NAVIGATION_ENTRY = {
...
describe('trackNavigationTimings', () => {
  ...
  it('notifies navigation timings after the load event', () => {
    const getNavigationEntrySpy = replaceMockableWithSpy(getNavigationEntry)
    // Use responseStart: 0 to avoid relativeNow() timing issues
    getNavigationEntrySpy.and.returnValue({
    ...
    expect(getNavigationEntrySpy).toHaveBeenCalled()
    expect(navigationTimingsCallback).toHaveBeenCalledOnceWith({
      firstByte: 0 as Duration,
  ...
  it('does not report "firstByte" if "responseStart" is negative', () => {
    replaceMockableWithSpy(getNavigationEntry).and.returnValue({
      ...FAKE_NAVIGATION_ENTRY,
  ...
  it('wait for the load event to provide navigation timing', () => {
    replaceMockableWithSpy(getNavigationEntry).and.returnValue(FAKE_NAVIGATION_ENTRY)
  ...
  it('discard incomplete navigation timing', () => {
    replaceMockableWithSpy(getNavigationEntry).and.returnValue(FAKE_INCOMPLETE_NAVIGATION_ENTRY)
    ;({ stop } = trackNavigationTimings(mockRumConfiguration(), navigationTimingsCallback))
