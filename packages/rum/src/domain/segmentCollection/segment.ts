...
export class Segment {
  ...
  addRecord(record: BrowserRecord, callback: () => void): void {
    ...
    this.writer.write(prefix + JSON.stringify(record), callback)
  }
  ...
  flush(callback: (metadata: BrowserSegmentMetadata) => void) {
    ...
    this.writer.write(`],${JSON.stringify(this.metadata).slice(1)}\n`, () => {
      replayStats.addWroteData(this.metadata.view.id, this.writer.rawBytesCount)
