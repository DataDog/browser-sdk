import { getInitCookie, isSyntheticsTest, mockable } from '@datadog/browser-core'
import { globalConsole, originalConsoleMethods } from '@datadog/js-core/util'
...
export function startAiAgentContext(assembleHook: AssembleHook) {
  ...
  assembleHook.register(({ eventType }): DefaultRumEventAttributes | SKIPPED => {
    ...
    return {
      ...
      context: {
        isAgentSession: true,
        aiAgentContext: staticDetection,
      },
    }
  })
}
