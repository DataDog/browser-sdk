...
import { nuxtRumPlugin, onRumInit, onRumStart, resetNuxtPlugin } from './nuxtPlugin'
...
describe('nuxtRumPlugin', () => {
  ...
  it('calls callbacks registered with onRumInit during onInit', () => {
    ...
    nuxtRumPlugin().onInit({ publicApi: PUBLIC_API, initConfiguration: INIT_CONFIGURATION })

    expect(spy).toHaveBeenCalledOnceWith(PUBLIC_API)
  })
