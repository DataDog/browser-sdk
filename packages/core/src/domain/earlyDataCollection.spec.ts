...
import { EarlyDataType, startEarlyDataCollection } from './earlyDataCollection'
import { ErrorHandling, ErrorSource, type RawError } from './error/error.types'

describe('startEarlyDataCollection', () => {
  it('collects runtime errors', (done) => {
    const runtimeErrorObservable = new Observable<RawError>()
    const { observable, stop } = startEarlyDataCollection(() => runtimeErrorObservable)
