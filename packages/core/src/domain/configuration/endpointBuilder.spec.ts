...
describe('endpointBuilder', () => {
  ...
  describe('proxy configuration', () => {
    it('should replace the intake endpoint by the proxy and set the intake path and parameters in the attribute ddforward', () => {
      expect(
        createEndpointBuilder({ ...initConfiguration, proxy: 'https://proxy.io/path' }, 'rum').build(
         ...
         new RegExp(`https://proxy.io/path\\?ddforward=${encodeURIComponent(
    ...
    it('normalizes the proxy url', () => {
      ...
      expect(endpoint.startsWith(`${location.origin}/path?ddforward`)).toBeTruthy()
    })
    ...
    it('should allow to fully control the proxy url', () => {
      ...
      expect(
        ...
        new RegExp(`https://proxy.io/prefix/api/v2/rum/suffix\\?ddsource=(.*)&dd-api-key=${clientToken}&dd-evp-origin-version=(.*)&dd-evp-origin=browser&dd-request-id=(.*)&batch_time=(.*)`)
      )
    })
  })
