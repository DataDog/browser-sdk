...
export function getCurrentSite(): string | undefined {
  if (getCurrentSiteCache === undefined) {
    const hostname = getCookieDefaultHostName()
    if (hostname) {
      // Use a unique cookie name to avoid issues when the SDK is initialized multiple times during
      // the test cookie lifetime
      ...
      const domainLevels = hostname.split('.')
...
function getCookieDefaultHostName() {
  try {
    return location.hostname || buildUrl(document.referrer).hostname
  } catch {
    // Ignore
  }
}
