...
import { EXPIRATION_DELAY, isValidSessionString, retrieveSession, SESSION_COOKIE_NAME } from './sessionManagement'
...
export function tryCookieMigration(sessionCookie: CookieCache) {
  ...
  if (isValidSessionString(sessionString)) {
    ...
    setCookie(SESSION_COOKIE_NAME, session.id || '', EXPIRATION_DELAY)
