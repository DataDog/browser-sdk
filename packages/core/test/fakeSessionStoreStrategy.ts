...
export function createFakeSessionStoreStrategy({
  ...
  return {
    ...
    persistSession: vi.fn<(newSession: SessionState) => void>().mockImplementation((newSession) => {
      session = newSession
    }),

    retrieveSession: vi.fn<() => SessionState>().mockImplementation(() => {
    ...
    expireSession: vi.fn<(previousSession: SessionState) => void>().mockImplementation((previousSession) => {
