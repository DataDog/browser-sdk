 ...
 * @param options.roles - Whether to keep or discard string role annotations in the serialized
 * output. The default is to discard them, which produces more readable output.
 * @param options.scope - A custom RecordingScope to use for serialization. (Takes
...
export async function serializeHtml(
  ...
  const scope = customScope ?? createRecordingScopeForTesting({ configuration })
  ...
  // Decode the full snapshot before we call after(). This ensures that the decoder has
  // already seen the string table that's been generated so far and can correctly decode
  // any additional records generated by after().
  const decoder = createChangeDecoder({
    keepRoles: roles === 'keep',
  })
  const decodedRecord = decoder.decode(emittedRecord)

  after?.(targetNode, scope, emittedStats, decoder)
