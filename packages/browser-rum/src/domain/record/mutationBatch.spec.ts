...
describe('createMutationBatch', () => {
  ...
  it('calls the callback asynchronously after MUTATION_PROCESS_MIN_DELAY', () => {
    ...
    expect(processMutationBatchSpy).toHaveBeenCalledExactlyOnceWith([mutation])
  })
  ...
  it('appends mutations to the batch when adding more mutations', () => {
    ...
    expect(processMutationBatchSpy).toHaveBeenCalledTimes(1)
    expect(processMutationBatchSpy).toHaveBeenCalledExactlyOnceWith([mutation1, mutation2, mutation3])
  })
  ...
  it('calls the callback on flush even if there is no pending mutation', () => {
    ...
    expect(processMutationBatchSpy).toHaveBeenCalledTimes(1)
    expect(processMutationBatchSpy).toHaveBeenCalledExactlyOnceWith([])
  })
})
