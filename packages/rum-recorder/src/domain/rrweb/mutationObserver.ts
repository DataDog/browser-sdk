...
/**
 * Use 'requestIdleCallback' when available: it will throttle the mutation processing if the browser
 * is busy rendering frames (ex: when frames are below 60fps). When not available, the fallback on
 * 'requestAnimationFrame' will still ensure the mutations are processed after any browser rendering
 * process (Layout, Recalculate Style, etc.), so we can serialize DOM nodes efficiently.
 */
const scheduleMutationProcess =
  ((window as unknown) as BrowserWindow).requestIdleCallback || window.requestAnimationFrame
const cancelMutationProcess = ((window as unknown) as BrowserWindow).cancelIdleCallback || window.cancelAnimationFrame
...
export function startMutationObserver(controller: MutationController, mutationCallback: MutationCallBack) {
      ...
      switch (state.status) {
        case MutationObserverStatus.WaitingForMutation:
          state = {
            ...
            scheduleId: scheduleMutationProcess(processPendingMutations, { timeout: MUTATION_PROCESS_MAX_DELAY }),
          }
          break
  ...
  function processPendingMutations() {
    ...
    if (state.status === MutationObserverStatus.MutationsPending) {
      ...
      cancelMutationProcess(state.scheduleId)
