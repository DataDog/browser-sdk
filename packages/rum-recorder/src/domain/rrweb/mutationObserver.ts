 ...
 */
export function startMutationObserver(controller: MutationController, emissionCallback: MutationCallBack) {
  const observer = new MutationObserver(monitor(handleMutations))
  ...
  controller.onFlush(() => handleMutations(observer.takeRecords()))

  function handleMutations(mutations: MutationRecord[]) {
    const payload = processMutations(mutations)
    if (payload) {
      emissionCallback(payload)
    }
  }
