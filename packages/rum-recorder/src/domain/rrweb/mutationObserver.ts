...
export function startMutationObserver(controller: MutationController, mutationCallback: MutationCallBack) {
  const observer = new MutationObserver(monitor((mutations) => processMutations(mutations, mutationCallback)))
  ...
  controller.onFlush(() => processMutations(observer.takeRecords(), mutationCallback))
