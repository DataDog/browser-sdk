import { NodePrivacyLevel, shouldMaskNode } from '../../privacy'
...
// Inlined from @datadog/browser-core isSafari
function isSafari(): boolean {
  const ua = window.navigator.userAgent
  return (
    (window.navigator.vendor?.indexOf('Apple') === 0 ||
      (/safari/i.test(ua) && !/chrome|android/i.test(ua))) &&
    !((window as any).chrome || /HeadlessChrome/.test(ua))
  )
}
