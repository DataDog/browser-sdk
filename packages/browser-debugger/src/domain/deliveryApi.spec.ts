...
describe('deliveryApi', () => {
  ...
  beforeEach(() => {
    ...
    clearProbes()
    clearDeliveryApiState()
    fetchSpy = vi.spyOn(window, 'fetch').mockImplementation(() => undefined as never)
    errorSpy = vi.spyOn(display, 'error').mockImplementation(() => undefined)
    warnSpy = vi.spyOn(display, 'warn').mockImplementation(() => undefined)
  ...
  describe('startDeliveryApiPolling', () => {
    ...
    it('should keep adding the other probes when one probe in the same response fails to initialize', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledExactlyOnceWith('Failed to add probe invalid:', expect.any(Error))
    })
