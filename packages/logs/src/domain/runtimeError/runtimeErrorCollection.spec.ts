import type { EarlyData, RawError } from '@datadog/browser-core'
import { ErrorSource, ErrorHandling, Observable, EarlyDataType, clocksNow } from '@datadog/browser-core'
...
function startRuntimeErrorCollectionWithDefaults({
  ...
  const earlyDataObservable = new Observable<EarlyData>()
  ...
  return { rawLogsEvents, earlyDataObservable }
}
...
describe('runtime error collection', () => {
  ...
  it('should send runtime errors', () => {
    const { rawLogsEvents, earlyDataObservable } = startRuntimeErrorCollectionWithDefaults()

    earlyDataObservable.notify({
